 ...
 * @version 1.0
 * 
 *          LogarithmicSpiralPlacer
 * 
 *          Places a word on the word map
 */
public class LogarithmicSpiralPlacer {
	...
	/*
	 * The basic algorithm for placing a word is taken from Wordle (see
	 * http://www.wordle.net). This implementation uses a logarithmic spiral to
	 * place the word. See https://en.wikipedia.org/wiki/Logarithmic_spiral
	 *
	 * Count the frequency of occurrence of each word and ignore ant commonly
	 * occurring words that are listed in an "ignore words" set. Sort the set of
	 * words in descending order by their frequency. Only retain the top n
	 * user-specified words . Assign each word a font size proportional to its
	 * frequency of occurrence. Paint the word onto a canvas using a Logarithmic or
	 * Archimedean sprial (Spira Mirabilis) as follows:
	 *
	 * Place the word where it wants to be While it intersects any of previously
	 * placed word Move it one step along an ever-increasing spiral
	 *
	 */
	public void place(WordFrequency wf) {
		...
		Font font = new Font("Tahoma", 0, wf.getFontSize()); // Create a font with a size proportional to the word
																// frequency
		...
		Rectangle word = new Rectangle(i, j - (int) (bounds.getHeight() * 0.8d), (int) bounds.getWidth(),
		...
		while (detector.collides(word, placed)) {
			...
			i = x;
			j = y;
			word = new Rectangle(i, j - (int) (bounds.getHeight() * 0.8d), (int) bounds.getWidth(),
					(int) bounds.getHeight());
			k++;
		}
